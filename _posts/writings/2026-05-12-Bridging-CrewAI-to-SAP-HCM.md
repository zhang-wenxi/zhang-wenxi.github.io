...
<div class="article">
  ...
  <p class="drop-cap">My current CrewAI project filters Singapore job listings for scam
  signals, scores them against a candidate CV, and surfaces skills gaps across three
  specialised agents. The agents are narrow, the output is structured JSON, and the
  pattern is stable. A reasonable next question is whether the same architecture could
  reach into SAP HCM for payroll queries, leave approvals, or org lookups. I reviewed
  three posts from SAP Community and the CrewAI forum to map out what that extension
  would actually require. I have not built the HCM side myself. What follows reflects
  the sources, not personal implementation.</p>
